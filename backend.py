"""
backend.py
Day 21 - Deployment of a Full Stack App

It wii be same as Day 20, with only two changes for deployment:
1. I am adding CORS middleware (so Streamlit Cloud frontend can call this API)
2. Uses --host 0.0.0.0 --port $PORT when started (set in railway.json/Procfile)
...
app = FastAPI(title="Day 21 - Deployed Full Stack App")

# Allow our Streamlit frontend (running on a different domain) to call this API.
# In production, replace "*" with your actual Streamlit app URL for tighter security.
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
@app.get("/")
def root():
    return {"message": "Day 21 API is running. Go to /docs to test it."}